...
#include "D3D12TimerCopy.h"
	...
	HRESULT D3D12TimerCopy::init(ID3D12Device* pDevice, UINT numTimers)
	{
		HRESULT hr = S_OK;
		device_ = pDevice;
		...
		if (SUCCEEDED(hr = device_->CreateQueryHeap(&queryHeapDesc, IID_PPV_ARGS(&queryHeap_))))
		{
			...
			{
				queryResourceCPU_->SetName(L"queryCopyResourceCPU_");
			}
			...
			{
				queryResourceGPU_->SetName(L"queryCopyResourceGPU_");
			}
		}
	...
	// Start timestamp.
	void D3D12TimerCopy::start(ID3D12GraphicsCommandList* pCommandList, UINT timestampPairIndex)
	{
		active_ = true;
	...
	// Stop timestamp.
	void D3D12TimerCopy::stop(ID3D12GraphicsCommandList* pCommandList, UINT timestampPairIndex)
	{
		active_ = false;
	...
	void D3D12TimerCopy::resolveQueryToCPU(ID3D12GraphicsCommandList* pCommandList, UINT timestampPairIndex)
	{
	...
	void D3D12TimerCopy::resolveQueryToCPU(ID3D12GraphicsCommandList* pCommandList, UINT timestampPairIndexFirst, UINT timestampPairIndexLast)
	{
	...
	void D3D12TimerCopy::resolveQueryToGPU(ID3D12GraphicsCommandList* pCommandList, ID3D12Resource** ppQueryResourceGPUOut)
	{
	...
	void D3D12TimerCopy::setGPUResourceState(ID3D12GraphicsCommandList* pCommandList, D3D12_RESOURCE_STATES prevState, D3D12_RESOURCE_STATES newState)
	{
	...
	GPUTimestampPair D3D12TimerCopy::getTimestampPair(UINT timestampPairIndex)
	{
		GPUTimestampPair p{};
	...
	void D3D12TimerCopy::calculateTime()
	{
		// Copy to CPU.
		UINT64 timeStamps[2];
		{
	...
	UINT64 D3D12TimerCopy::getDeltaTime()
	{
		return deltaTime_;
	}

	UINT64 D3D12TimerCopy::getEndTime()
	{
		return endTime_;
	}

	UINT64 D3D12TimerCopy::getBeginTime()
	{
		return beginTime_;
	}

	// Whether timer is active.
	bool D3D12TimerCopy::isActive()
	{
		return active_;
	}