...
Texture::Texture(std::wstring filePath, ID3D12Device1* device)
{
	this->Create(filePath, device);
}
...
bool Texture::Create(std::wstring filePath, ID3D12Device1* device)
{
	...
	if (wicFactory == NULL)
	{
		...
		if (FAILED(hr))
		{
			Log::PrintError(Log::ErrorType::ENGINE, "Failed to \'coCreateInstance\' when loading texture from file.\n");
			return false;
		}
	}
	...
	hr = wicFactory->CreateDecoderFromFilename(
		this->filePath.c_str(),                        // Image we want to load in
	...
	if (FAILED(hr))
	{
		Log::PrintError(Log::ErrorType::ENGINE, "Failed to \'CreateDecoderFromFilename\' when loading texture from file.\n");
		return false;
	}
	...
	if (FAILED(hr))
	{
		Log::PrintError(Log::ErrorType::ENGINE, "Failed to \'GetFrame\' when loading texture from file.\n");
		return false;
	}
	...
	if (FAILED(hr))
	{
		Log::PrintError(Log::ErrorType::ENGINE, "Failed to \'GetPixelFormat\' when loading texture from file.\n");
		return false;
	}
	...
	if (FAILED(hr))
	{
		Log::PrintError(Log::ErrorType::ENGINE, "Failed to \'GetSize\' when loading texture from file.\n");
		return false;
	}
	...
	if (dxgiFormat == DXGI_FORMAT_UNKNOWN)
	{
		...
		if (FAILED(hr))
		{
			Log::PrintError(Log::ErrorType::ENGINE, "Failed to \'CreateFormatConverter\' when loading texture from file.\n");
			return false;
		}
		...
		{
			Log::PrintError(Log::ErrorType::ENGINE, "Failed to \'canConvert\' when loading texture from file.\n");
			return false;
		}
		...
		if (FAILED(hr))
		{
			Log::PrintError(Log::ErrorType::ENGINE, "Failed to \'Initialize\' when loading texture from file.\n");
			return false;
		}
	...
	this->CreateDefaultHeap(device);
}

void Texture::CreateDefaultHeap(ID3D12Device1* device)
{
